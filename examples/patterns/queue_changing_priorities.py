...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 25 15:40:51 2024
...
start_event(shop, [], [to_split], "customer_arrived", lambda: exp(1/8), behavior=lambda: [SimToken(2)])
...
#task(shop, [repair_queue, worker, packing_queue.queue], [wait_sync_w_clean, worker,packing_queue.queue], "repair", start_repair, guard=priority_condition)
...
#task(shop, [clean_queue, worker, packing_queue.queue], [wait_sync_w_rep, worker, packing_queue.queue], "clean", start_cleaning, guard=priority_condition)
...
task(shop, [packing_queue, worker],[to_done, worker], "packing", start_packing)
